...
randsup=2.015
d7 <- d5 + geom_area(data = subset(db5, x > randsup), aes(x=x, y=y), fill="red")+
  rlt_theme
...
## El limite superior
Evaluando las variables necesarias se necesita solamente el valor de t con la tipo de error I de 95% y que corresponde a un tamaño de muestra de 20, $t_{0.05,\ n-1}$. Para conocer este valor hay que ir a una tabla de la distribución de t (vea p. 217 en el libro) o usar R para calcular el valor.  En la tabla busca el error deseado (0.05) y se busca el grado de libertad *n-1*, $t_{0.05,\ 19}$. El valor del libro es $t = 2.093$.  
 
```{r}
qt(0.025, 19) # nota que se usa la mitad de 0.05, por que es una prueba de dos lados. 
```
 
 Ahora falta sustituir los valores en la formula

$UL_{0.95}=\overline{x}+\left(t_{0.05,\ n-1}\cdot\frac{s}{\sqrt{n}}\right)$
$$UL_{0.95}=20+2.093*\left(\frac{1.76}{\sqrt{20}}\right)=21.825$$
## El limite inferior

$LL_{0.95}=\overline{x}-\left(t_{0.05,\ n-1}\cdot\frac{s}{\sqrt{n}}\right)$


$$LL_{0.95}=20-2.093*\left(\frac{1.76}{\sqrt{20}}\right)=20.175$$
...
```{r}
UL=20+qt(0.025, 4)*1.76/sqrt(5)
LL=20-qt(0.025, 4)*1.76/sqrt(5)
UL
LL
```
...
```{r}
UL=20+qt(0.025, 99)*1.76/sqrt(100)
LL=20-qt(0.025, 99)*1.76/sqrt(100)
UL
LL
```
...
```{r}
t=(196.7-209.4)/((39.1)/sqrt(10000)) 
t # Valor t de observado
qt(0.025, 9999)  # valor de t-critico, de la tabla
```
...
El resultado:  El valor de t-observado es de 5.625, con un grado de libertad de 243 (n=244), y valor de p <0.0001. 
Por consecuencia se rechaza la hipótesis nula y se acepta la alterna.  El intervalo de confianza del promedio es $2.82 - $3.17, con un promedio de $3.00
...
1. Crear una distribución normal de la hipótesis **NULA** con un promedio de 
2. Calcule las estadísticas t.test
3. Dibuje la media de la estadística t en la figura
  - la linea entecortada representa el intervalo de confianza de 95%
  - la linea roja representa la estadistica de los datos.
