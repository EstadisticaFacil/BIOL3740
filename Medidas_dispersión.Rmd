...
es_a= sd(a$a)/sqrt(length(a$a))

es_b= sd(b$b)/sqrt(length(b$b))
...
```{r}
Limite_inferior_a=mean(a$a)-(es_a+1.96)
Limite_superior_a=mean(a$a)+(es_a+1.96)

Limite_inferior_a
Limite_superior_a

Limite_inferior_b=mean(b$b)-(es_b+1.96)
Limite_superior_b=mean(b$b)+(es_b+1.96)
...
Col%>%
ggplot(aes(Col))+
  geom_density(fill="blue", colour="white",alpha=.5)+
  theme(legend.position = "none")+
...
geom_vline(aes(xintercept = as.numeric(promCol-3*sdCol)), colour="orange")+
  geom_vline(aes(xintercept = as.numeric(promCol+3*sdCol)), colour="orange")+ scale_x_continuous(expand=c(0,0), limit=lims, 
...
Para evaluar el 95% de intervalo de confianza usaremos datos de la alturas de 500 adultos. Estos datos fueron bajado del siguiente website <https://www.kaggle.com/yersever/500-person-gender-height-weight-bodymassindex/data>
...
promM=Alturas_Humanos%>%
  select(Genero, Altura_cm)%>%
  filter(Genero=="Mujer")%>%
  summarize(Mean=mean(Altura_cm))
...
ggplot(aes(Altura_cm))+
  geom_histogram(fill="blue", colour="white",alpha=.5)+
  theme(legend.position = "none")+
...
## Ahora prepara el mismo gráfico para los hombres.  
